...
try {
    ...
    try {
        ...
        if (addPayment($data) !== false && $data['payment_status'] === 'approved') {
            ...
            header("location:http://localhost/zelina.tech/payment/paypal/PaypalSuccess.php?payid=$inserids");
            exit(1);
        } else {
            // Payment failed
			header("location:http://localhost/zelina.tech/payment/paypal/PaypalFailed.php");
             exit(1);
        }
